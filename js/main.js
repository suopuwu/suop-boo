document.addEventListener('DOMContentLoaded', () => {
    let infoSections = [
        ...
        },
        {
            ...
            paragraphs: [
                `<img class="lightbox-target" src="img/transcriber1.png"> In a sentence, this is a wrapper around whisper.cpp, sherpa-onnx, and llama.cpp designed to sand off the rough edges I was experiencing when trying to document my life and remember the good times via audio recordings. When writing it, I wanted an app that was only on when its window was open + wasn't an electron app, so I settled on QT. It's obvious from the external dependencies that this is a program that allows one to transcribe, diarize, and summarize their audio files, so what makes this one different? Real world convenience features.`,
                `<img class="lightbox-target" src="img/transcriber2.png">Audio - transcript integration -- This is not a program that you drop your audio into and it spits out transcripts. The idea behind it was to somewhat mimic an Obsidian vault, where the audio files and transcriptions live next to each other, highlighting the currently spoken line as the audio plays. It allows the user to easily access both in the same screen. Transcripts are md files, so you can open them in whatever program you want.`,
                `<img class="lightbox-target" src="img/transcriber3.png">Diarization -- Speakers can be auto recognized, defaulting to speaker 1, speaker 2, etc. Once done, you can rename / combine speakers, then filter across all transcripts for those speakers.`,
                `<img class="lightbox-target" src="img/transcriber4.png">Auto labeling, summarization, chaptering, tagging -- This feature uses the built in LLM to analyze the transcripts after they are made. Human readable labels display over filenames, a one paragraph summary blurb is generated at the start of transcripts, chapters are auto created with timestamps, and tags that can be filtered by are auto generated. All of these features are optional and only run when the user requests them (though you can run a batch with all enabled.)`,
                `<img class="lightbox-target" src="img/transcriber5.png">Recorder -- This feature allows you to record your own computer's audio + whatever mic you are using. You can adjust individual app / mic volume. Useful for if you want to record a call with someone while you're on your computer.`,
            ],
        },
        {
